...
const Profile = () => {
    ...
    useEffect(() => {
        ...
        const savedOrders = JSON.parse(localStorage.getItem('farmer_orders') || '[]');
        setOrders(savedOrders.sort((a, b) => b.timestamp - a.timestamp));
    }, []);
            ...
            <div className="container">
                <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'grid', gap: '2rem' }}>
                        ...
                        <div style={{ background: 'white', padding: '2rem', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', marginBottom: '4rem' }}>
                            <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem', borderBottom: '1px solid #f3f4f6', paddingBottom: '0.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                {t('order_history.title')}
                                <span style={{ fontSize: '0.9rem', color: '#6b7280', fontWeight: 'normal' }}>{orders.length} {t('dashboard.orders_count')}</span>
                            </h2>

                            {orders.length === 0 ? (
                                <div style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>
                                    <p>{t('order_history.no_orders')}</p>
                                </div>
                            ) : (
                                <div style={{ display: 'grid', gap: '1rem' }}>
                                    {orders.map(order => (
                                        <div key={order.orderId} style={{
                                            ...
                                            <div>
                                                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '0.25rem' }}>
                                                    <span style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>{order.supplier || 'Ceylon Seeds Ltd.'}</span>
                                                    {order.remainingBalance === 0 ? (
                                                        <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '0.7rem', background: '#dcfce7', color: '#166534', fontWeight: '600' }}>✅ Full Payment OK</span>
                                                    ) : (
                                                        <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '0.7rem', background: '#fef9c3', color: '#854d0e', fontWeight: '600' }}>⏳ Pending</span>
                                                    )}
                                                </div>
                                                <div style={{ fontSize: '0.85rem', color: '#6b7280' }}>
                                                    <span>{t('order_history.table_id')}: {order.orderId}</span>
                                                    <span style={{ margin: '0 8px' }}>•</span>
                                                    <span>{new Date(order.timestamp).toLocaleDateString()}</span>
                                                </div>
                                            </div>
                                            <div style={{ textAlign: 'right' }}>
                                                <div style={{ fontWeight: '800', fontSize: '1.2rem', color: '#166534', marginBottom: '0.5rem' }}>
